...
function geodir_buddypress_pagination_aui( $before = '', $after = '', $prelabel = '', $nxtlabel = '', $pages_to_show = 5, $always_show = false ) {
    ...
    if ( empty ( $prelabel ) ) {
        $prelabel = '<span class="nav-prev-text sr-only">' . __( 'Previous', 'geodir-buddypress' ) . '</span> <i class="fas fa-chevron-left"></i>';
    }

    $user_domain = geodir_buddypress_get_user_domain();

    if (empty($nxtlabel)) {
        $nxtlabel = '<span class="nav-next-text sr-only">' . __( 'Next', 'geodir-buddypress' ) . '</span> <i class="fas fa-chevron-right"></i>';
    }
