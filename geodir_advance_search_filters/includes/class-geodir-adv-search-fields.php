...
class GeoDir_Adv_Search_Fields {
	...
	public static function advance_search_button( $instance = array() ) {
		...
		if ( $rows > 0) {
			...
			if ( $fa_class ) {
				...
				$button_content = '<i class="' . esc_attr( $button_text ) . '" aria-hidden="true"></i><span class="sr-only">' . $aria_label . '</span>';
			} else {
	...
	public static function output_main_categories( $html, $cf, $post_type ) {
		...
		if ( $design_style ) {
			$html .= "<div class='gd-search-field-taxonomy gd-search-field-categories col-auto flex-fill'" . geodir_search_conditional_field_attrs( $cf, '', 'select' ) . ">";
	...
	public static function output_main_datepicker( $html, $field, $post_type ) {
		...
		if($design_style ){
			if ( $field->search_condition == 'FROM' ) {
				...
				?>
				<div class="<?php echo ( $has_fieldset ? 'form-group ' : '' ); ?>gd-search-has-date gd-search-<?php echo $htmlvar_name; ?> from-to col-auto flex-fill"<?php echo geodir_search_conditional_field_attrs( $cf ); ?>>
				...
				?>
				<div class="<?php echo ( $has_fieldset ? 'form-group ' : '' ); ?>gd-search-has-date gd-search-<?php echo $htmlvar_name; ?> col-auto flex-fill"<?php echo geodir_search_conditional_field_attrs( $cf ); ?>>
					<?php if ( ! empty( $field_label ) ) { ?>
						<label for="<?php echo esc_attr( $htmlvar_name ); ?>" class="sr-only"><?php echo $field_label; ?></label>
					<?php }
	...
	public static function output_main_time( $html, $field, $post_type ) {
		...
		if($design_style){
			if ( $field->search_condition == 'FROM' ) {
				...
				?>
				<div class="<?php echo ( $has_fieldset ? 'form-group ' : '' ); ?>gd-search-has-time gd-search-<?php echo $htmlvar_name; ?> from-to col-auto flex-fill"<?php echo geodir_search_conditional_field_attrs( $cf ); ?>>
					<?php if ( ! empty( $field_label) ) { ?>
						<label for="<?php echo esc_attr( $htmlvar_name ); ?>" class="sr-only"><?php echo $field_label; ?></label>
					<?php }
					echo aui()->input(
				...
				$field_value         = ! empty( $field_value ) && ! is_array( $field_value ) ? sanitize_text_field( $field_value ) : '';
				?>
				<div class="<?php echo ( $has_fieldset ? 'form-group ' : '' ); ?>gd-search-has-time gd-search-<?php echo $htmlvar_name; ?> col-auto flex-fill"<?php echo geodir_search_conditional_field_attrs( $cf ); ?>>
					<?php if ( ! empty( $field_label) ) { ?>
						<label for="<?php echo esc_attr( $htmlvar_name ); ?>" class="sr-only"><?php echo $field_label; ?></label>
					<?php }
	...
	public static function output_main_business_hours( $html, $cf, $post_type ) {
		...
		if ( $design_style ) {
			$html .= "<div class='gd-search-input-wrapper gd-search-field-cpt gd-search-" . $cf->htmlvar_name . " col-auto flex-fill'" . geodir_search_conditional_field_attrs( $cf ) . ">";
			$html .=  aui()->select( array(
	...
	public static function output_main_distance( $html, $cf, $post_type ) {
		...
		if ( $dist > 0 ) {
			...
			$values = '<input type="hidden" name="dist" value="'  . $dist .  '"><input type="hidden" name="_unit" value="'  . esc_attr( $unit ) .  '" data-ignore-rule>';
			$popover_label_class = '';
		} else {
		...
		$button_content = '<i class="fas fa-bullseye' . $icon_class . '" aria-hidden="true"></i><span class="' . $range_class . '" title="' . esc_attr( $distance_title ) . '">' . $distance_value . '</span><span class="sr-only">' . $label . '</span>';
		...
		if ( $as_fieldset_start > 0 ) {
			...
			if ( $design_style ) {
				...
				if ( $geodir_search_advanced ) {
					$class = ' form-group col-auto flex-fill';
					$style = '';
				} else {
					$class = ' col mb-3';
				...
				if ( $field_info->field_type != 'checkbox' ) {
					...
					$html .= '<label ' . $label_for . ' class="text-muted form-field-label">' . stripslashes( __( $field_label, 'geodirectory' ) ) . '</label>';
				}
			} else {
				$field_label = $field_info->frontend_title ? $field_info->frontend_title : $field_info->admin_title;
				$htmlvar     = $field_info->htmlvar_name;
			...
			if ( $field->search_condition == 'FROM' ) {
				...
				<div class="<?php echo ( $has_fieldset ? 'form-group ' : '' ); ?>gd-search-has-date gd-search-<?php echo $htmlvar_name; ?> from-to">
					<?php if ( ! empty( $as_fieldset_start ) ) { ?>
						<label for="<?php echo esc_attr( $htmlvar_name ); ?>" class="text-muted form-field-label"><?php echo $field_label; ?></label>
					<?php }
	...
	public static function output_field_distance( $html, $field_info, $post_type ) {
		...
		if ( $field_info->search_condition == "RADIO" ) {
			...
			if ( $field_info->htmlvar_name == 'distance' && $field_info->extra_fields != '' ) {
				...
				if ( $sort_options != '' ) {
					echo '<ul><li class="' . esc_attr( $field_class ) . '">' . $display_label . '<select id="" class="cat_select' . ( geodir_design_style() ? ' custom-select form-control' : '' ) . '" name="sort_by" data-ignore-rule>';
	...
	public static function output_field_fieldset( $html, $field_info, $post_type ) {
		...
		if ( $design_style ) {
			...
			if ( $as_fieldset_start == 0 ) {
				...
				$html .= '<label class="text-muted form-field-label' . $fieldset_class . '">' . stripslashes( __( $field_label, 'geodirectory' ) ) . '</label>';
			} else {
				$as_fieldset_start ++;
				$field_label = $field_info->frontend_title ? $field_info->frontend_title : $field_info->admin_title;
				$htmlvar     = $field_info->htmlvar_name;
	...
	public static function output_field_multiselect( $html, $field_info, $post_type ) {
		...
		if ( in_array( $field_info->input_type, array(
			...
			// optgroup
			$terms = ! empty( $select_fields_result->option_values ) ? geodir_string_values_to_options( $select_fields_result->option_values, true ) : array();
		} else {
			$terms = explode( ',', $select_fields_result->option_values );
		}
		...
		if ( $design_style ) {
			...
			<div class="geodir-filter-container<?php echo $container_class; ?>">
				<div class="geodir-more-filters customize_filter-in clearfix <?php echo $filters_class; ?>">
					<div class="customize_filter_inner text-left <?php echo $inner_class; ?>">
					...
					<div class="geodir-advance-search">
						...
						echo str_replace( "col-auto", $button_class, ob_get_clean() );
						?>
					</div>
				</div>
			</div>
