...
class GeoDir_Adv_Search_Admin_Conditional_Fields {
	...
	public function conditional_fields_setting( $field_type, $field, $data, $key ) {
		...
		<div class="aui-conditional-field geodir-con-fields-hidden border-top mt-4 pt-4" data-setting="conditional_fields_heading" data-element-require="!jQuery(form).find('#gd_main_search<?php echo esc_attr( $key ); ?>:checked').length">
			<a href="#geodir_conditional_fields" data-toggle="collapse" class="btn btn-block btn-outline-primary"><span class="geodir-show-cf"><i class="fas fa-plus" aria-hidden="true"></i></span><span class="geodir-hide-cf"><i class="fas fa-minus" aria-hidden="true"></i></span> <?php _e( 'Conditional Fields', 'geodiradvancesearch' ); echo $count_badge; ?></a>
		</div>
		<div class="collapse" data-setting="conditional_fields" id="geodir_conditional_fields">
			...
			$if = '<div class="input-group-prepend"><span class="input-group-text border-left-0 px-2">'.__( 'if', 'geodiradvancesearch' ) . '</span></div>';
			?>
